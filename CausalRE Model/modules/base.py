...
'''
This is not used anymore, all moved to model
'''




class ModelBase(nn.Module, PyTorchModelHubMixin):
    '''
    This is the base class for the model, they put some basic common code and some structure code in here, just to make things more confusing for anyone reading this stuff
    some notes:
    they read in the config
    they instatitate teh data processor
    set the tokensplitter
    enforce the need for a forward method
    define the followining methods in full, I guess they are model agnostic:
    - adjust_logits
    - predict
    - evaluate
    - create_dataloader => whcih just calls the cretae_data_loader from the data processor
    - save pretrained => save model to disk or upload to hf hub
    - _load_pretrained => overload the hf function I think
    '''
    ...
    #@abstrctmethod here just means all classes inheriting from this class need to implement forward
    @abstractmethod
    def forward(self, x):
        pass
    ...
    #i am not sure abotu all this stuff, I think it is due to hugging face
    def save_pretrained(
            self,
