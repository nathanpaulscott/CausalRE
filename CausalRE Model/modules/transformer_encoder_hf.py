...
class TransformerEncoderHF_w_prompt(torch.nn.Module):
    '''
    ...
    def transformer_encoder(self, tokens: List[List[str]], seq_lengths: torch.Tensor):
        ...
        return input_ids, embeddings, masks, cls_reps, w2sw_map
    ...
    def forward(self, x, mode=None):
        ...
        #run the modified inputs through the HF encoder transformer
        input_ids, embeddings, masks, cls_reps, w2sw_map = self.transformer_encoder(prompt_x, len_prompt_x)
        
        #split the embeddings to: token_reps, token_masks, span_type_reps, rel_type_reps
        #NOTE: token_reps and token_masks will be aligned with subword tokens if subtoken_pooling is 'none', o/w they will be aligned with word tokens
        #NOTE: !!!VIP!!! we update the w2sw_map for the non-pooling case so the map is for non-cls-sep and non prompt word tokens to subword tokens
        result = self.prompt_proc.split_embeddings(embeddings, 
                                                   masks, 
                                                   prompt_len, 
                                                   span_prompt_len,
                                                   w2sw_map,        #needed as we update this to account for removal of prompt
                                                   input_ids,       #used for the no pooling HF case only
                                                   prompt_x,        #used for debug only
                                                   self.tokenizer)  #used for debug only
        ...
        sw_span_ids = self.make_sw_span_ids(x['span_ids'], w2sw_map)
        ...
        return dict(token_reps     = token_reps, 
                    ...
                    cls_reps       = cls_reps,      #will only have value if we are using HF
                    sw_span_ids    = sw_span_ids,   #will only have value if we are in nonpooling mode for HF
                    w2sw_map       = w2sw_map)      #will only have value if we are in nonpooling mode for HF
