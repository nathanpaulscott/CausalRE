...
###############################################
#custom imports
from .layers_transformer_encoder_hf_w_prompt import TransformerEncoderHFPrompt
from .layers_filtering import FilteringLayerBinaryDouble, FilteringLayerBinarySingle
from .layers_other import LstmSeq2SeqEncoder, TransformerEncoderTorch, GraphEmbedder, OutputLayerPrompt, OutputLayer
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        if self.config.use_prompt:
            self.output_head_span = OutputLayerPrompt(input_size  = self.config.hidden_size,
                                                      dropout     = self.config.dropout)
            self.output_head_rel = OutputLayerPrompt(input_size   = self.config.hidden_size,
                                                     dropout      = self.config.dropout)
