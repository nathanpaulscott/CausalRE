...
from .rel_processor_spert import RelationProcessor
from .rel_rep import RelationRepLayer
from .utils import clear_gpu_tensors


'''
Mod this to emulate spert
i.e.
only have 2 layers => span and rel classification heads
The span head is wehre the span fitler head would be
Therel generation only uses the positively predicted spans from teh span head for the expansion
The spans and rels use random neg sampling (spans pure random, rels => random sample from strong rels only)
rel gen has an upper limit (say 1000-2000)
Overall, shoudl not se so different to what is there now, just the rel gen and neg sampling which is mainly different
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        #hardcode the prompt to always be False
        self.config.use_prompt = False
        ######################################
        ...
        if self.config.backbone_model_source == 'flair':
            self.transformer_encoder_w_prompt = TransformerEncoderFlairPrompt(self.config)
        elif self.config.backbone_model_source == 'HF':
    ...
    def calc_pred_losses(self, logits_span, cand_span_labels, cand_span_masks, 
        ...
        #span pred loss
        class_weights = self.config.pos_case_unilabel_class_weight
        if isinstance(class_weights, (float, int)):
            class_weights = self.config.span_class_weights        
        ...
        if self.config.rel_labels  == 'multilabel':
            class_weights = self.config.pos_case_multilabel_class_weight
        if isinstance(class_weights, (float, int)):
            class_weights = self.config.rel_class_weights        
    ...
    def forward(self, x, step=None):
        ...
        #set the binary_class_weights for the binary filter heads
        binary_class_weights = self.config.pos_case_binary_class_weight   #set to the config first
        if isinstance(binary_class_weights, (float, int)):   #if it is a number, then it would have been converted to a weights tensor, so read that
            binary_class_weights = self.config.binary_class_weights        
        ...
        if bypass_span_filter:   #this is a random neg sampling for testing
            ...
            force_pos_cases = self.set_force_pos('span', step) if self.training else False
        ...
        rel_ids, rel_masks, rel_labels, lost_rel_counts, lost_rel_penalty = self.rel_processor.get_cand_rel_tensors(cand_span_masks,       #the span mask for each selected span  (batch, top_k_spans)
                                                                                                                    ...
                                                                                                                    #x, top_k_spans, span_idx_to_keep, sw_span_ids, filter_score_span, span_masks, span_labels) #the mapping from span_ids to cand_span_ids for each batch item (batch, num_spans)
        ...
        force_pos_cases = self.set_force_pos('rel', step) if self.training else False
        ...
        else:    #if we bypass the graph completely
            node_reps, edge_reps = cand_span_reps, cand_rel_reps
