...
class TransformerEncoderHFPrompt(torch.nn.Module):
    ...
    def make_w2sw_tensor(self, w2sw_map, max_seq_len):
        '''
        Creates a tensor where each row corresponds to a word in the input sequence,
        with columns representing the first and last subword indices.
        NOTE: we do not add 1 to the end here, these are actual mappings
        i.e. 
        if start word token 4 maps to sw tokens 8,9,10, thus the start sw will be 8
        if end word token 8 maps to sw tokens 12,13,14,15 => the end sw will be 15
        so if we had a span with start and end of 4 and 9, then the sw start would be the sw result for w token 4, whcih is 8
        and the sw end would be the result for 9 - 1 => w token 8 for end => 15, then we add 1 
        '''
        batch_size = len(w2sw_map)
        #make w2sw_tensor
        w2sw_tensor = torch.full((batch_size, max_seq_len + self.config.max_span_width, 2), -1, device=self.device, dtype = torch.long)
        for i, map_dict in enumerate(w2sw_map):
            for word_id, sw_ids in map_dict.items():
    ...
    def make_sw_span_ids_vectorized(self, span_ids, w2sw_map, max_seq_len):
        ...
        #make the w2sw_tensor
        w2sw_tensor = self.make_w2sw_tensor(w2sw_map, max_seq_len)
        ...
        sw_start_indices = torch.gather(w2sw_tensor[:, :, 0], 1, start_indices)
        sw_end_indices = torch.gather(w2sw_tensor[:, :, 1], 1, end_indices) + 1
        ...
        '''
        this mostly works, but the issue is teh invalid spans, either ignore or do as I have done just expand
        but expanding makes sw_span_ids erroneous in length right?
