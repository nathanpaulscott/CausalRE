...
class RelationProcessor():
    ...
    def get_rel_labels(self, raw_rels, orig_map, span_to_cand_span_map):  #, x, t, sik, swsids, fss, sm, sl):
        ...
        for i in range(self.batch):
            for head_idx_raw, tail_idx_raw, rel_type in raw_rels[i]:
                ...
                if head_cand_idx != -1 and tail_cand_idx != -1:
                    ...
                    print(orig_map[i])
                    print(span_to_cand_span_map[i]) 
                    for id,  (sid, swsid, score, cand_span_idx, mask, label) in enumerate(zip(x['span_ids'][i], swsids[i], fss[i], span_to_cand_span_map[i], sm[i], sl[i])):
                        msg = f'{id}: {x["tokens"][i][sid[0]] if sid[0] < len(x["tokens"][i]) else "---"}:{x["tokens"][i][sid[1]-1] if sid[1]-1 < len(x["tokens"][i]) else "---"} => {sid.tolist()}, {swsid.tolist()}, {round(score.item(),4)}, {cand_span_idx.item()}, {mask.item()}, {label.item()}'
    ...
    def get_cand_rel_tensors(self, cand_span_masks, raw_rels, orig_map, span_to_cand_span_map):    #), x, t, sik, swsids, fss, sm, sl):
        """
        ...
        rel_labels, lost_rel_counts, lost_rel_penalty = self.get_rel_labels(raw_rels, orig_map, span_to_cand_span_map)   #, x, t, sik, swsids, fss, sm, sl)
