...
class FilteringLayerBinaryDouble(nn.Module):
    ...
    def forward(self, reps, masks, binary_labels, force_pos_cases=True, reduction='sum', class_weights=None, loss_only=False):
        """
        ...
        - class_weights: either a tensor of class weights or 'calc' or 'none' or None
        ...
        if binary_labels is not None:
            ...
            filter_loss = cross_entropy_loss(logits, binary_labels, masks, reduction=reduction, class_weights=class_weights)
...
class FilteringLayerBinarySingle(nn.Module):
    ...
    def forward(self, reps, masks, binary_labels, force_pos_cases=True, reduction='sum', class_weights=None, loss_only=False):
        ...
        if binary_labels is not None:
            #Compute the loss if in training mode
            #NOTE: the logits and labels are flattened and reduction is sum, so the loss output is one scalar for all spans/rels in all obs in the batch
            filter_loss = binary_cross_entropy_loss(logits, binary_labels, masks, reduction=reduction, class_weights=class_weights)
