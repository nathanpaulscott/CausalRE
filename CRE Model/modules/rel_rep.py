...
class RelRepContextBase(nn.Module, ABC):
    ...
    def make_context_masks(self, token_masks, cand_span_ids, **kwargs):
        '''
        ...
        batch, top_k_spans, _ = cand_span_ids.shape
    ...
    def forward(self, cand_span_reps, cand_span_ids, token_reps, token_masks, neg_limit, **kwargs):
        ...
        """
        batch, top_k_spans, _ = cand_span_reps.shape  # (batch, top_k_spans, hidden)
        #Span representation expansion for relational context.
        head_reps, tail_reps = make_head_tail_reps(cand_span_reps, top_k_spans)
        #Generate the context mask (subclass-specific)
        context_masks = self.make_context_masks(token_masks, cand_span_ids, **kwargs)
