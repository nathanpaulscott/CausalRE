...
class RelRepNoContext(nn.Module):
    ...
    def forward_old(self, span_reps, rel_ids, span_filter_map, pruned_rels=False, **kwargs):
        ...
        """
        if pruned_rels:
            #Map rel_ids to span_ids
            mapped_rel_ids = map_rel_ids_to_span_ids(rel_ids, span_filter_map)
            rel_reps = make_head_tail_rel_reps_from_pruned_ids(span_reps, mapped_rel_ids)
            return self.out_layer(rel_reps)    #(batch, pruned_rels, hidden)
...
class RelRepContextBase(nn.Module, ABC):
    ...
    def forward(self, span_reps, span_ids, token_reps, token_masks, neg_limit, rel_ids, span_filter_map, pruned_rels=False, **kwargs):
        """
        Computes relation representations by concatenating the head/tail and algo specific context reps
        """
        if pruned_rels:
            mapped_rel_ids = map_rel_ids_to_span_ids(rel_ids, span_filter_map)
            headtail_reps = make_head_tail_rel_reps_from_pruned_ids(span_reps, mapped_rel_ids)    #(batch, pruned_rels, hidden*2)
            raise Exception('the context reps generation code is not yet working for the prune rels case, I need time to work on it')
            context_reps = make_context_rel_reps_from_pruned_ids(mapped_rel_ids, token_reps, token_masks, neg_limit, self.make_context_masks, self.make_context_reps)   #(batch, pruned_rels, hidden)
            #Concatenate head, tail, and context representations
            rel_reps = torch.cat([headtail_reps, context_reps], dim=-1)  # (batch, num_rel, hidden_size * 3)
