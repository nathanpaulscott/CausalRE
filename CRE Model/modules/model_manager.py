...
from .model_nathan1 import Model as model_nathan1
#from .model_spert import Model as model_spert
#from .model_nathan2 import ModelNathan2
...
class ModelManager:
    ...
    def get_model(self, device=None):
        ...
        #make the model
        if self.config.model_name == 'nathan1':
            model = model_nathan1(self.config).to(device)
        elif self.config.model_name == 'spert':
...
class Optimizer:
    ...
    def get_optimizer_params_alt(self, model, config):
        ...
        encoder_params = list(model.transformer_encoder_w_prompt.parameters())
    ...
    def get_optimizer_params(self, model, config):
        ...
        if config.freeze_encoder:
            ...
            for param in model.transformer_encoder_w_prompt.parameters():
                param.requires_grad = False
        else:
            encoder_params = list(model.transformer_encoder_w_prompt.named_parameters())
        ...
        for name, layer in model.named_children():
            if name != "transformer_encoder_w_prompt":
