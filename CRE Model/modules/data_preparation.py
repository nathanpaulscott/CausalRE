...
class DataPreparation:
    ...
    def make_type_mappings_and_span_ids(self):
        ...
        #make the class weights if required
        if config.pos_case_binary_class_weight not in ['none', 'calc']:
            if config.filter_head_type == 'double':   #we need a tensor with both neg and pos case weights
                binary_class_weights = torch.tensor([1] + [config.pos_case_binary_class_weight], dtype=config.torch_precision, device=config.device)
            else:   #we just need a single valued tensor for the pos case weight
                binary_class_weights = torch.tensor([config.pos_case_binary_class_weight], dtype=config.torch_precision, device=config.device)
            self.main_configs.update(dict(binary_class_weights = binary_class_weights))
        
        if config.pos_case_unilabel_class_weight not in ['none', 'calc']:
            span_class_weights = torch.tensor([1] + [config.pos_case_unilabel_class_weight]*(num_span_types-1), dtype=config.torch_precision, device=config.device)
            self.main_configs.update(dict(span_class_weights = span_class_weights))
            if config.rel_labels  == 'unilabel': 
                rel_class_weights = torch.tensor([1] + [config.pos_case_unilabel_class_weight]*(num_rel_types-1), dtype=config.torch_precision, device=config.device)
                self.main_configs.update(dict(rel_class_weights = rel_class_weights))

        if config.rel_labels  == 'multilabel' and config.pos_case_unilabel_class_weight not in ['none', 'calc']: 
            rel_class_weights = torch.tensor([config.pos_case_multilabel_class_weight], dtype=config.torch_precision, device=config.device)
            self.main_configs.update(dict(rel_class_weights = rel_class_weights))
